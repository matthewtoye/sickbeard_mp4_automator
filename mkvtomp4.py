...
class MkvtoMp4:
    ...
    def generateOptions(self, inputfile, original=None):
        ...
        if '10le' in info.video.pix_fmt or '16le' in info.video.pix_fmt:
            if 'nvenc_h264' in self.video_codec: #nvenc_h264 requires yuv420p output when accepting a 10/12 bit HEVC stream. nvenc_hevc handles this without issues as of 3/20/2017
                use_yuv420p = True
